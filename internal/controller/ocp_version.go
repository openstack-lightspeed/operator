...
func GetOCPIndexName(version string) string {
	...
	if version == OCPVersionLatest {
		return fmt.Sprintf("%s-%s", OpenStackLightspeedOCPIndexPrefix, OCPVersionLatest)
	}

	// For version numbers, replace dots with underscores
...
// RAGConfig represents a single RAG source configuration
